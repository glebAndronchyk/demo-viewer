...
# Type-check all packages (run per-package — root -r flag doesn't work)
bun --filter @demo-viewer/domain type-check
bun --filter @demo-viewer/api type-check
...
- New domain operations require: a Command, a Handler, and registration in `backend/domain/src/handlers/index.ts` (auto-wired by `buildOperations`). Exporting from the index is sufficient — no other wiring needed.
- The API's OpenAPI docs are auto-generated by `@elysiajs/openapi` and served at `http://localhost:<port>/openapi`.
- Always duplicate your plans in `docs/plans`.
- Domain entity fields must be camelCase, even when the corresponding DB schema uses snake_case. Conversion happens in mappers only.

### Analytics Calculators

`backend/domain/src/operations/analytics/` contains per-match calculators (one per analytics category). All extend `AnalyticsCalculator<TReturn>` and take `(matchId, playerSteamId, matchOutbound)`. Call `.calculate()` to get the result. The base class provides a two-tier cache: `dbCache` for raw event queries, `operationCache` for computed metrics. Use `@AnalyticsCalculator.cache()` on individual methods.

Calculators are instantiated directly by handlers — they are not registered as commands themselves.
