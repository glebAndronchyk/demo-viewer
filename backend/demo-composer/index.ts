...
const parse = (args: ParseArgs): { pid: number; promise: Promise<number> } => {
  const baseArgs = Boolean(args.filePath)
    ? [parserExec, "-demo", args.filePath ?? "", "-chunk-size", String(args.framesInChunkCount)]
    : [parserExec, "-url", args.fileUrl ?? "", "-chunk-size", String(args.framesInChunkCount)];

  if (args.shareCode) {
    baseArgs.push("-share-code", args.shareCode);
  }

  const child = Bun.spawn(baseArgs, {
    env: { ...process.env },
